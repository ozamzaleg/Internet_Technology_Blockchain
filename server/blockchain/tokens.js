...
class Block {
    constructor(index, transactions, prev, nonce = 0) {
        this.index = index;
        this.nonce = nonce;
    ...
    calculateHash() {
        return SHA256(this.index + JSON.stringify(this.transactions) + this.prev + this.nonce).toString();
    }
...
class Blockchain {
    ...
    addBlock(block) {
        const newBlock = new Block(this.chain.length + 1, block.transactions);
    ...
    calculateBlocksHash(index) {
        ...
        this.chain.slice(index).map((block, i) => {
            ...
            currentBlock = new Block(block.index, block.transactions, prev, block.nonce);
    ...
    mineBlockCalculateHash(index) {
        const currentBlock = new Block(this.chain[index].index, this.chain[index].transactions, this.chain[index].prev);
